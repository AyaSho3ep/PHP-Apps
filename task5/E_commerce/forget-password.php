...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($validation->getErrors())){
        ...
        $user = new User;
        $user->setVerification_code($passCode)->setEmail($_POST['email']);
        
        if($user->updateCode()){
            $body = "<p> Hello {$_POST['email']} </p> 
            <p> Your Forget Password Code :<b style='color:blue;'> {$passCode} </b> </p>
            <p> Ecommerce Team.</p>";
            $mailVerificationCode = new VerificationCode($_POST['email'],"Verification Code Mail",$body);
            if($mailVerificationCode->send()){
